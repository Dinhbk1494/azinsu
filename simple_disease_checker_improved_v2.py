...
# Convert new pattern system to old PREFIX_DEFS format for backward compatibility
def _generate_prefix_defs():
    """Generate PREFIX_DEFS from new pattern system to maintain compatibility"""
    prefix_defs = []
    
    # Main drug patterns (treat role) - highest priority
    for pattern in MAIN_DRUG_PATTERNS:
        # Convert negative lookbehind patterns to simpler prefixes for compatibility
        if "điều" in pattern:
            prefix_defs.extend([
                {'role': 'treat', 'prefix': r'(?<!hỗ trợ )(?<!phòng ngừa )(?<!đề phòng )(điều\s*trị|chữa\s*trị|trị\s*liệu)'},
                
            ])
    
    # Secondary drug patterns (support role)
    if SECONDARY_DRUG_PATTERNS:
        prefix_defs.append({'role': 'support', 'prefix': r'(hỗ\s*trợ|phòng\s*ngừa|đề\s*phòng)(?!và điều trị)(?!hoặc điều trị)'})
        prefix_defs.append({'role': 'support', 'prefix': r'hỗ\s*trợ điều\s*trị'})
        prefix_defs.append({'role': 'support', 'prefix': r'(giúp\s*giảm|làm\s*giảm)(?!và điều trị)(?!hoặc điều trị)'})
        
    
    # Add combined patterns for better matching
    prefix_defs.extend([
    ...
    print("Generated PREFIX_DEFS: ", prefix_defs)
    return prefix_defs

# Generate PREFIX_DEFS from new system
PREFIX_DEFS = _generate_prefix_defs()
print("PREFIX_DEFS: ", PREFIX_DEFS)
...
def find_matching_diseases(disease_name: str) -> list:
    """Find diseases that match based on keywords (at least 2 common keywords for precision)."""
    ...
    for disease_key, disease_data in DISEASE_VARIANTS.items():
        ...
        # Special case: if only 1 keyword but it's a very specific match
        elif len(common_words) == 1 and len(main_keywords) == 1:
            matching_diseases.append(disease_key)
            if DEBUG:
                print(f"Matched '{disease_name}' -> '{disease_key}' (single specific keyword: {common_words})")
...
def _build_patterns_for_one_disease(disease: str) -> list:
    ...
    for core in variants:
        for pd in PREFIX_DEFS:
            ...
            role = pd['role']
            if pfx:
                pattern = rf"{pfx}.{{0,7}}{core}"
            else:
                pattern = rf"{core}"
            patterns.append(pattern)
    print("______________build_patterns_for_one_disease: ", disease, "-> patterns: ", patterns)
    return patterns
...
def _cached_pattern_entries(diseases_key: tuple) -> list:
    ...
    for d in diseases_key:
        ...
        print("00000000000 variants: ", variants[0])
        for core in variants:
            print("11111111111 PREFIX_DEFS: ", PREFIX_DEFS[0])
            for pd in PREFIX_DEFS:
                pfx = pd['prefix']
                role = pd['role']
                if pfx:
                    patt = rf"{pfx}.{{0,7}}{core}"
                    print("patt with prefix: ", patt)
                else:
                    patt = rf"{core}"
                try:
                    entries.append({
...
async def check_drug_list_by_multi_diseases_ilike(diagnosis_text: str, drug_names, database_url: str = None, use_llm_filter: bool = False, pre_split_diseases: list = None):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            rows = await conn.fetch(query, ilike_patterns)
            ...
            num_drug_matches_by_ilike   = 0
            list_temp_drug = []
            list_temp_2 = set()
            for row in rows:
                # if 'Bilclamos' in row['name']:
                # print("========== Debug hit Ileffexime row: ", (row))
                print("input name: ", input_names)
                for name in input_names:
                    if name.lower() in str(row['name']).lower():
                        list_temp_2.add(name)
                        print("====== Match by ILIKE: ", name, " in ", row['name'])
                    else:
                        print("====== No match by ILIKE: ", name, " in ", row['name'])
                    # if name not in list_temp_drug:
                    #     if (xxx not in row['name'] for xxx in list_temp_drug):
                    #         list_temp_drug.append(name)
                    #         print("aaaaaaaa: ", list_temp_drug[-1], " bbbbbb ", row['name'])
                    #         list_temp_2.append(list_temp_drug[-1])
                    #         num_drug_matches_by_ilike += 1
                print("====== list_temp_drug: ", list_temp_drug)
                print("====== list_temp_2: ", list_temp_2)
                ...
                for input_name in input_names:
                    ...
                    if row_name_lower.find(input_name.lower()) != -1:
                        ...
                        for ent in entries:
                            # print('======== text: ', text)
                            # print("========ent['compiled']: ", ent['compiled'])
            ...
            print((f"Số lượng thuốc được tìm thấy qua ILIKE: {num_drug_matches_by_ilike}, Tổng số  thuốc cần kiểm tra: {len(input_names)}"))
            async def _fetch_fuzzy(one_name: str):
                try:
                    return one_name, await conn.fetch(fuzzy_query, one_name)
                except Exception:
                    return one_name, []
            time_start = time.time()
